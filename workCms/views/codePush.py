...
import os, paramiko, traceback, logging

from .auxiliay import session_zsq, dates
from workCms import status_restful_api
from .database import ssh_port
from workCms.modeCls.securityModule import Security

codepush = Blueprint('codepush', __name__)

auditFileDir = "workCms/serviceall/commandAudit.txt"
cmdFileDir = "workCms/serviceall/cmdResult.txt"


def linuxAudit(ipAndHostname: str, code: str):
    s = '******'
    with open(auditFileDir, 'a', encoding='utf8') as f:
        f.write(f"执行时间: {dates}{s}远程服务器地址: [{ipAndHostname}]{s}执行代码: [{code}]\n")
        f.close()
...
def catCmdRes():
    ...
    resAll = cmdRes_all_text.split('\n')
    f.close()
    return render_template('cmdRes.html', cmdResInfo=resAll)
...
def linuxHandle():
    if request.method == 'GET':
        ...
        if errCode == True:
            print('代码已被阻止执行 -> ', code)
            logging.error(dates + '代码已被阻止执行 -> ' + code)
